...
class UserRegisterForm(forms.ModelForm, UserFieldsPrepareMixin):
    ...
    def clean(self):
        ...
        if (
            ...
            or cleaned_data["email"] == ""
            or cleaned_data["nickname"] == ""
            or cleaned_data["password"] == ""
            or cleaned_data["password_confirmation"] == ""
        ):
        ...
        if cleaned_data["password"] != cleaned_data["password_confirmation"]:
            ...
            raise forms.ValidationError(
                "Пользователь с таким логином не может существовать."
            )
...
class UserProfileForm(forms.Form, UserFieldsPrepareMixin):
    ...
    avatar = forms.FileField(
        required=False, validators=UserProfile._meta.get_field("avatar").validators
    )
    ...
    def clean(self):
        ...
        if not has_changes:
            raise forms.ValidationError(
                "Вы должны изменить хотя бы одно поле или загрузить аватар."
            )
