...
class PaginationManager:
    ...
    def __init__(
        ...
        objs_per_page: int = DEFAULT_RECORDS_PER_PAGE,
    ):
        try:
    ...
    def first_page(self):
        return (
            self.page_range.start if self.page_range.start != self.range.start else None
        )

    @property
    def last_page(self):
        return (
            self.page_range.stop - 1
            if self.range.stop != self.page_range.stop
            else None
        )
