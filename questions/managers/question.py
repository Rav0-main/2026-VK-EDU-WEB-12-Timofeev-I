...
class QuestionManager(models.Manager):
    def get_questions_with_tag(self, tag_name: str, user=None) -> QuerySet:
        return (
            self.__form_question_query_set(user)
    ...
    def get_answers(
        self, question_id: int, user=None
    ) -> "QuerySet[Answer] | list[Answer]":
        try:
            answers = self.get(id=question_id).answers

            votes_subquery = models.Subquery(
                AnswerLike.objects.filter(answer=models.OuterRef("pk"))
                .values("answer")
            ...
            if user:
                user_liked_subquery = models.Subquery(
                    AnswerLike.objects.filter(
                        answer=models.OuterRef("pk"), author=user
                    ).values("type")[:1]
                )
    ...
    def __form_question_query_set(self, user=None) -> QuerySet:
        vote_subquery = models.Subquery(
            QuestionLike.objects.filter(question=models.OuterRef("pk"))
            .values("question")
            .annotate(total=models.Sum("type"))
            .values("total")
        )
        ...
        answers_subquery = models.Subquery(
            Answer.objects.filter(question=models.OuterRef("pk"))
            .values("question")
        ...
        if user:
            user_liked_subquery = models.Subquery(
                QuestionLike.objects.filter(
                    question=models.OuterRef("pk"), author=user
                ).values("type")[:1]
            )
